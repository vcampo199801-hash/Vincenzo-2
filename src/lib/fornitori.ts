 ...
 * usata per i contratti del personale. Rilevante solo se è stata inserita una
 * scadenza da monitorare: niente casella "attivo" separata, la presenza
 * stessa della data è il segnale. */
export function contrattoFornitoreStato(scadenzaContratto: Date | null | undefined): {
  giorni: number | null;
  stato: ContrattoFornitoreStato;
} {
  if (!scadenzaContratto) return { giorni: null, stato: "OK" };
